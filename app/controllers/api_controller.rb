...
class ApiController < ApplicationController
  def signup  #ovo je kontroler za signup preko API-a
    ...
    #return render json:  unless client_app
    if (!client_app)
      return render json: {"value"=>{}, "error"=> true, "errorCode"=>"Invalid client ID"}
    end
    ...
    if user.save
        ...
        render json: {"value"=> var, "error" => false, "errorCode" => "no error"}
    else
      #render(json: { error: user.errors.full_messages }, status: 422)
      render json: {"value"=>{}, "error"=>true, "errorCode"=>user.errors.full_messages}
    end
  end
